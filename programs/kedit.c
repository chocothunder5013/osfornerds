...
void draw_ui() {
    clear_screen();
    printf("--- KEDIT: %s (Ctrl+S/tilde to Save, Ctrl+Q/backtick to Quit) ---\n\n", filename);
...
int main(int argc, char **argv) {
    // Temporary fallback until the kernel supports passing argv
    if (argc < 2) {
        filename = "test.txt";
        printf("Warning: No argv support yet. Defaulting to %s\n", filename);
        // Pause so the user can see the warning
    ...
    if (fd != -1) {
        ...
        if (buffer_size < 0)
            buffer_size = 0; // Prevent corruption on read error
        close(fd);
    } else {
        buffer_size = 0;
    }
