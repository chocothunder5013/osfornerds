...
int is_valid_user_ptr(void *ptr, int size) {
    ...
    // --- THE FIX ---
    // Our shell is a Ring 0 Kernel Thread right now. Its strings live in kernel
    // memory (~2MB). We must bypass this check so the shell can print!
    // In the future, you will check if (current_process->cr3 == user_cr3) here.
