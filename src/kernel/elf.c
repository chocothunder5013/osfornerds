...
/*
 * Parses an Executable and Linkable Format (ELF) file and prepares it for execution.
 * It verifies the ELF magic number, creates a new virtual address space (page directory),
 * iterates through the program headers to find PT_LOAD segments, and maps physical memory
 * for those segments. It copies the segment data from the file into memory, zeroes out
 * any uninitialized data (BSS), and finally spawns a new process pointing to the ELF entry point.
...
int elf_load_file(const char *filename, char *args) {
    ...
    return create_process((void (*)())hdr->entry, args, highest_addr, 0, (uint32_t)new_pd);
}