...
void system_monitor_task() {
    ...
    while (1) {
        counter++;
        if (counter % 500000 == 0) { // Lowered the count so it doesn't block too long
            serial_log(" [BG-TASK] System Alive. Tick...\n");
            counter = 0;

            // --- NEW: Give the CPU back to the scheduler! ---
            sys_yield();
        }
    }
}
...
void compositor_task() {
    while (1) {
        ...
        // CRITICAL: We must yield so the shell and background tasks can run!
        sys_yield();
    }
}
...
void kmain(multiboot_info_t *mboot_ptr, uint32_t magic) {
    ...
    create_process(compositor_task, 0, 0, 1);
    ...
    while (1) {
        ...
        // Let the CPU rest and give time to the shell/programs
