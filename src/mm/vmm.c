...
void vmm_map_page_in_dir(page_directory_t *dir, void *phys, void *virt, int flags) {
    ...
    if (!(dir->tablesPhysical[pdindex] & I86_PTE_PRESENT)) {
        ...
        dir->tablesPhysical[pdindex] =
            (uint32_t)new_pt_phys | I86_PTE_PRESENT | I86_PTE_WRITABLE | I86_PTE_USER;
    }

    uint32_t  pt_phys = dir->tablesPhysical[pdindex] & 0xFFFFF000;
    uint32_t *pt_virt =
        (uint32_t *)pt_phys; // NOTE: In a real higher-half kernel, we would need to map this
                             // temporarily. Since we identity map low memory, this works for now.
...
page_directory_t *vmm_create_address_space() {
    ...
    // Critical: We share kernel tables, we do NOT copy the pages themselves, just the pointers to
    // tables.
    for (int i = 0; i < 32; i++) {
