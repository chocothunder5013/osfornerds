...
void mouse_handler() {
    ...
    // --- NEW: The QEMU Desync Fix ---
    // If we are expecting the first byte (flags), but bit 3 is 0,
    // the packet is out of sync. Throw it away!
    ...
    if (mouse_cycle == 3) {
        ...
        // The magic of int8_t: It automatically handles the negative math for us!
